...
set path = ($path /bin /sbin /usr/bin /usr/local/bin /usr/local/sbin /usr/home)


# Set the path of the script and log file path (monitor with: tail -f uptime-kuma-gateway-status.log)
set script_name = 'uptimekuma-opnsense-gw-status'
set script_path = '/usr/home/uptimekuma-opnsense-gw-status/'
...
while ( 1 == 1 )
    ...
    foreach wan ( $gateway_names )
        ...
        set uptime_kuma_url = `(awk -v FS="," -v wan="$wan" '$1 == wan && $4 ~ /./ {print $4}' options.txt)`
